...
class PdfFormView extends StatefulWidget {
  final EditableInvoice? editableInvoice;
...
class _PdfFormViewState extends State<PdfFormView>
  ...
  // Controllers for items and paid items
  List<InvoiceItem> _items = [];
  List<PaidItem> _paidItems = [];
  ...
  void dispose() {
    ...
    for (var paidItem in _paidItems) {
      paidItem.dispose();
    }
    super.dispose();
  }
  ...
  double _calculatePaidItemAmount(PaidItem item) {
    return double.tryParse(item.amountPaidController.text) ?? 0;
  }

  double _calculateTotalPaid() {
    return _paidItems.fold(0, (sum, item) => sum + _calculatePaidItemAmount(item));
  }
  ...
  void _removePaidItem(int index) {
    setState(() {
      _paidItems[index].dispose();
      _paidItems.removeAt(index);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    child: TextFormField(
    controller:
    _paidItems[index].productController,
    decoration: const InputDecoration(
    labelText: 'Paid Product',
    border: OutlineInputBorder(),
    ),
    ),
    ),
    ...
    child: TextFormField(
    controller:
    _paidItems[index].quantityController,
    decoration: const InputDecoration(
    labelText: 'Qty',
    border: OutlineInputBorder(),
    ),
    ...
    child: TextFormField(
    controller:
    _paidItems[index].amountPaidController,
    decoration: const InputDecoration(
    ...
    IconButton(
    icon: const Icon(Icons.delete),
    onPressed: () => _removePaidItem(index),
    color: Colors.red,
    ),
    ],
    ),
    ...
    Text(
    _formatCurrency(_calculateTotal() - _calculateTotalPaid()),
    style: const TextStyle(
    fontSize: 20,
    fontWeight: FontWeight.bold,
    color: Colors.blue,
        ...
        onPressed: () async {
          if (_formKey.currentState!.validate()) {
            ...
            try {
              ...
              // Prepare paid items
              List<Map<String, dynamic>> paidItems = _paidItems.map((item) {
                return {
                  'product': item.productController.text,
                  'quantity':
                  int.tryParse(item.quantityController.text) ?? 0,
                  'amountPaid':
                  double.tryParse(item.amountPaidController.text) ?? 0,
                };
              }).toList();
              ...
              final file = await _pdfService.generatePdf(
                ...
                paidAmount: _calculateTotalPaid(),
...
class PaidItem {
  ...
  PaidItem({
    ...
    TextEditingController? amountPaidController,
        ...
        quantityController = quantityController ?? TextEditingController(),
        amountPaidController = amountPaidController ?? TextEditingController();
